...
recode_age_epi <- function(df) {
  ...
  ageEPI2 <- ifelse(df$age / (365.25 / 12) >= 2 & df$age / (365.25 / 12) < 4 & bcg == 1 & penta1 == 1 & polio1 == 1, 1,
               ifelse(df$age / (365.25 / 12) < 2 | df$age / (365.25 / 12) >= 4, NA, 0))
  ## Age appropriate immunisation status - 4 months to less than 6 months
  ageEPI3 <- ifelse(df$age / (365.25 / 12) >= 4 & df$age / (365.25 / 12) < 6 & bcg == 1 & penta1 == 1 & penta2 == 1 & polio1 == 1 & polio2 == 1, 1,
                    ifelse(df$age / (365.25 / 12) < 4 | df$age / (365.25 / 12) >= 6, NA, 0))
  ## Age appropriate immunisation status - 6 months to less than 9 months
  ageEPI4 <- ifelse(df$age / (365.25 / 12) >= 6 & df$age / (365.25 / 12) < 9 & bcg == 1 & penta1 == 1 & penta2 == 1 & penta3 == 1 & polio1 == 1 & polio2 == 1 & polio3 == 1, 1,
                    ifelse(df$age / (365.25 / 12) < 6 | df$age / (365.25 / 12) >= 9, NA, 0))
  ## Age appropriate immunisation status - 9 months to less than 18 months
  ageEPI5 <- ifelse(df$age / (365.25 / 12) >= 9 & df$age / (365.25 / 12) < 18 & bcg == 1 & penta1 == 1 & penta2 == 1 & penta3 == 1 & polio1 == 1 & polio2 == 1 & polio3 == 1 & measles1 == 1 & rubella == 1, 1,
                    ifelse(df$age / (365.25 / 12) < 9 | df$age / (365.25 / 12) >= 18, NA, 0))
  ## Age appropriate immunisation status - 18 months or more
  ageEPI6 <- ifelse(df$age / (365.25 / 12) >= 18 & bcg == 1 & penta1 == 1 & penta2 == 1 & penta3 == 1 & polio1 == 1 & polio2 == 1 & polio3 == 1 & measles1 == 1 & measles2 == 1 & rubella == 1, 1,
                    ifelse(df$age / (365.25 / 12) < 18, NA, 0))
  ##
  ageImm <- rowSums(cbind(ageEPI1, ageEPI2, ageEPI3, ageEPI4, ageEPI5, ageEPI6), na.rm = TRUE)
...
recode_birthweight <- function(df) {
  ...
  ##
  lbw <- bbw::recode(var = bwtCard, recodes = "lo:2499=1;NA=NA;else=0")
  ## concatenate data.frame
  bwt <- data.frame(bwt, bwtCard, lbw)
  ##
  return(bwt)
}
...
recode_chealth <- function(df,
  ...
  ## Recode diarrhoea
  dia <- recode_diarrhoea(df = df)
  ## Recode cough/ARI
  ari <- recode_ari(df = df)
  ## Recode fever
  fev <- recode_fever(df = df)
