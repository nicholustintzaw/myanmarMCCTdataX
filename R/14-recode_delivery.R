...
recode_birth <- function(df,
                         core.columns = c("KEY",
                                          ...
                                          "sample_component")) {
  ## Where delivered?
  birth1 <- as.integer(df$deliv_place)
  ...
  gestmonths <- as.integer(df$deliv_gestmonth)
  gestmonths <- bbw::recode(var = gestmonths, recodes = "444=NA;999=NA")
  ## Gestational age at birth - weeks
  gestweeks <- as.integer(df$deliv_gestweek)
  ...
  birth2 <- as.integer(df$deliv_place_why)
  ## Why: convenience
  birth2a <- as.integer(df$deliv_place_why.1)
  birth2a <- ifelse(is.na(birth2), NA, birth2a)
  ## Why: tradition
  birth2b <- as.integer(df$deliv_place_why.2)
  birth2b <- ifelse(is.na(birth2), NA, birth2b)
  ## Why: close distance
  birth2c <- as.integer(df$deliv_place_why.3)
  birth2c <- ifelse(is.na(birth2), NA, birth2c)
  ## Why: safety for mother/baby
  birth2d <- as.integer(df$deliv_place_why.4)
  birth2d <- ifelse(is.na(birth2), NA, birth2d)
  ## Why: affordable cost
  birth2e <- as.integer(df$deliv_place_why.5)
  birth2e <- ifelse(is.na(birth2), NA, birth2e)
  ## Who assisted in delivery?
  birth3 <- as.integer(df$deliv_assist)
  ## Who: doctor
  ...
  ## Delivery method
  birth4 <- as.integer(df$deliv_method)
  ## method: normal
  ...
  ## delivery costs?
  birth5 <- as.integer(df$deliv_cost)
  ## deliverty costs amount
  birth6 <- as.integer(df$deliv_cost_amount)
  birth6 <- bbw::recode(var = birth6, recodes = "444=NA;666=NA;999=NA")
  ## Borrow?
  birth7 <- as.integer(df$deliv_cost_loan)
