################################################################################
#
#' Function to create dataset with MCCT-related data from the Myanmar MCCT
...
#'   MCCT data
#'
#' @return A data.frame of recoded MCCT-related indicators for children
#'
#' @examples
...
create_mcct <- function(df, x) {
  x1 <- df[ , c("KEY", "geo_rural", "geo_state", "sample_component", "intrv_date",
                names(df)[stringr::str_detect(string = names(df), pattern = "mcct")])]
  ##
